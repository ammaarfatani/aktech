...
export function Marquee() {
  ...
  return (
    <div className="relative w-full overflow-hidden bg-[#060816] py-10 border-y border-white/[0.04] z-10">
      {/* Side Vignette Fades */}
      <div className="absolute inset-y-0 left-0 w-24 bg-gradient-to-r from-[#060816] to-transparent z-20 pointer-events-none" />
      <div className="absolute inset-y-0 right-0 w-24 bg-gradient-to-l from-[#060816] to-transparent z-20 pointer-events-none" />
      ...
      <div className="flex w-[200%] gap-12 sm:gap-20 items-center animate-[marquee_30s_linear_infinite] hover:[animation-play-state:paused] whitespace-nowrap">
        {marqueeItems.map((logo, idx) => (
          <div
            key={idx}
            className="flex items-center gap-3.5 grayscale opacity-35 hover:grayscale-0 hover:opacity-90 hover:scale-105 transition-all duration-500 cursor-pointer"
          >
            <img
              src={logo.url}
              alt={logo.name}
            ...
            />
            <span className="text-xs sm:text-sm font-heading font-semibold text-gray-300 tracking-widest uppercase">
              {logo.name}
            </span>
          </div>
        ))}
